...
| **RQ3** | Do static performance smells predict measured energy cost? *(Deferred — requires Linux RAPL; see Section 7)* |
...
**Answer to RQ2:** No consistent "AI is worse" signal exists on this benchmark when measured by static finding counts. The oracle results (Section 5.4) confirm the correct framing: AI code is functionally plausible but security-insecure, not statically noisier.

---

### 5.3 RQ4 — Auto-Fix Effectiveness
...
### 5.4 RQ5 — Baseline Comparison (Core Result)
...
### 5.5 RQ5 (continued) — CWEval Oracle Pass Rates
...
### 5.6 RQ6 — Secure@k and Vulnerable@k
